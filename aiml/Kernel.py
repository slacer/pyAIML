...
class Kernel:
	...
	def __init__(self):
		...
		self._logger = logging.getLogger("pyaiml.kernel")
	...
	def bootstrap(self, brainFile = None, learnFiles = [], commands = []):
		...
		if not isinstance(learnFiles, (list, tuple)):
			learnFiles = [ learnFiles ]
		for lf in learnFiles:
			self.learn(lf)
		...
		self._logger.info("Kernel bootstrap completed in %.2f seconds" % (time.clock() - start))
	...
	def loadBrain(self, filename):
		...
		"""
		self._logger.info("Loading brain from %s" % filename)
		start = time.clock()
		self._brain.restore(filename)
		end = time.clock() - start
		self._logger.info("Finished loading brain from %s (%d categories in %.2f seconds)" % (filename, self._brain.numTemplates(), end))

	def saveBrain(self, filename):
		"""Dump the contents of the bot's brain to a file on disk."""
		self._logger.info("Saving brain to %s" % filename)
		start = time.clock()
		self._brain.save(filename)
		self._logger.info("Finished saving brain to %s (%.2f seconds)" % (filename, time.clock() - start))
	...
	def _respond(self, input, sessionID):
		...
		if len(inputStack) > self._maxRecursionDepth:
			self._logger.warning("Maximum recursion depth exceeded (input='%s')" % input.encode(self._textEncoding, 'replace'))
			return ""
		...
		if elem is None:
			self._logger.warning("No match found for input: %s" % input.encode(self._textEncoding))
		else:
	...
	def _processElement(self,elem, sessionID):
		...
		try:
			...
			# type!
			self._logger.warning("No handler found for <%s> element" % elem[0].encode(self._textEncoding, 'replace'))
			return ""
			...
			try:
				...
				for li in listitems:
					try:
						...
						self._logger.info("Something amiss -- skipping listitem " + str(li))
						raise
				if not foundMatch:
					...
					try:
						...
						self._logger.warning("Error in default listitem")
						raise
			except:
				# Some other catastrophic cataclysm
				self._logger.warning("Catastrophic condition failure")
				raise
		return response
	...
	def _processInput(self, elem, sessionID):
		...
		try: return inputHistory[-index]
			...
			self._logger.error("No such index %d while processing <input> element." % index)
			return ""
	...
	def _processSystem(self,elem, sessionID):
		...
		try:
			...
			self._logger.error("RuntimeError while processing \"system\" element: %s" % msg.encode(self._textEncoding, 'replace'))
	...
	def _processThat(self,elem, sessionID):
		...
		try:
			...
			self._logger.error("No such index %d while processing <that> element." % index)
			return ""
...
if __name__ == "__main__":
	logging.getLogger('pyaiml').addHandler(logging.NullHandler())
	# Run some self-tests
	k = Kernel()
